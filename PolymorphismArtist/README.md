...
This project demonstrates polymorphism in Java through the use of an abstract base class called `Shape`, three concrete subclasses, and an `Artist` class that interacts with them. The Shape class defines a common method called `drawYourself()`, which is overridden by the `Square`, `Circle`, and `Triangle` subclasses to provide their own specific behavior. While the method name remains the same across all classes, the output differs depending on which object is being referenced.

The `PolymorphismArtist` class acts as the main driver of the program, where each shape is created using the parent Shape type. These shape objects are then passed to an `Artist` object, which is responsible for calling `drawYourself()` on each shape. The structure of this project reflects how polymorphism works in practice. Even though the variables are declared using the same parent type, Java determines which version of the method to execute at runtime based on the actual object being instantiated. When the program runs, each shape prints a unique message to the console, confirming that the correct overridden method is being executed.
...
The `Artist` class represents the object that interacts with the shapes. Instead of knowing which specific shape it is working with, the `Artist` only works with the `Shape` type and calls `drawYourself()` on it. This helps demonstrate polymorphism more clearly, since the `Artist` does not need to know whether it is drawing a square, circle, or triangle. The `PolymorphismArtist` class contains the main method, where the shape objects and the artist are created and tested. Together, these classes demonstrate the relationship between abstraction, inheritance, and polymorphism in Java.
